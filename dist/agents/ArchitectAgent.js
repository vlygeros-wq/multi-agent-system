...
class ArchitectAgent extends AgentBase_1.AgentBase {
    constructor(bus, idOverride) {
        const profile = {
            id: idOverride ?? (0, ids_1.generateAgentId)("architect"),
...
Your outputs are always structured, detailed, and include concrete tech-stack recommendations.`,
    ...
    async handleMessage(message) {
        switch (message.type) {
            case "task-assignment": {
                ...
                if (!task) {
                    logger_1.logger.warn(this.role, "Received task-assignment without task payload.");
                    return;
                }
            ...
            case "inter-agent": {
                logger_1.logger.info(this.role, `Inter-agent query from ${message.sender}: "${message.content}"`);
    ...
    async executeTask(task) {
        ...
        await this.send("orchestrator", "status-update", `Starting task: ${task.title}`, { taskId: task.id });
        await sleep(300);
...
ADR-004: Terraform for IaC — reproducible, version-controlled infrastructure.
    ...
    answerArchitecturalQuery(question) {
